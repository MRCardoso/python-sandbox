...
class Terrain(Entity):
    ...
    heightmap = None
    def generate(self, name: str, size: int, scale: float, octaves: float, persistence: float, lacunarity: float, seed: float, absolute=False) -> NDArray[np.float64]:
        print(f"- Generate Terrain[{name}]")
        terrain = np.zeros((size, size))
        for x in range(size):
            progress_bar(((x + 1) / size) * 100)
            for y in range(size):
                nx = x / scale
                ny = y / scale
                value = pnoise2(
